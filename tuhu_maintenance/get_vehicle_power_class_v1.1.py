...
def collect_vehicle_power_class(driver,collected_data):
    ...
    for letter in CarZiMus:
        if len(letter.text) == 1 and letter.text in ['L', 'X']:
            letter.click()  # 点击字母
            time.sleep(2)
            ...
            for brand in CarBrands:
                ...
                for series_index in range(len(Series)):
                    ...
                    try:
                        ...
                        # 插入数据库，只是缺少排量之后信息
                        result_list = [OriginalBrand, Brand, SeriesID, SeriesName, SeriesAndFactory, Factory, '-', '-', '-']
                        insert_into_MySQL(result_list)
